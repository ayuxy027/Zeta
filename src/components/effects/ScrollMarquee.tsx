...
export function ScrollVelocityContainer({
    ...
    return (
        <ScrollVelocityContext.Provider value={velocityFactor}>
            <div className={cn("overflow-visible relative w-full", className)} {...props}>
                {children}
            </div>
...
function ScrollVelocityRowImpl({
    ...
    return (
        <div
            ref={containerRef}
            className={cn("overflow-x-hidden overflow-y-visible w-full whitespace-nowrap", className)}
            {...props}
            style={{ paddingTop: '0.2em', paddingBottom: '0.2em' }}
        >
            <motion.div
                className="inline-flex items-baseline transform-gpu select-none will-change-transform"
                style={{ x }}
            >
